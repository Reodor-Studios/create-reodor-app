...
export function ChatMessage({
  ...
  const hasToolActivity =
    toolExecutionParts.length > 0 || toolCallParts.length > 0;
  ...
  const confirmationRequestsFromExecutions = toolExecutionParts
    .filter(
      (part: any) => part.result && part.result.requiresConfirmation === true
    )
    .map((part: any, index) => ({
  ...
  const clarificationRequestsFromExecutions = toolExecutionParts
    .filter(
      (part: any) => part.result && part.result.requiresClarification === true
    )
    .map((part: any, index) => ({
      toolCallId: `exec-${index}`,
        ...
        {shouldShowChainOfThought && (
          <ChainOfThought defaultOpen={status === "streaming" && isLast}>
            <ChainOfThoughtHeader>
              ...
              {toolExecutionParts.length + toolCallParts.length === 1
                ? "call"
                : "calls"}
              )
            </ChainOfThoughtHeader>
            <ChainOfThoughtContent>
              ...
              {toolExecutionParts.map((toolExecution, index) => {
                ...
                const isCurrentlyExecuting =
                  status === "streaming" && isLast && !hasOutput;
            ...
            {confirmationRequests.map((req, i) => (
              ...
              >
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="rounded-md bg-white dark:bg-amber-950/50 p-3 space-y-1">
                      {Object.entries(req.action.details).map(
                        ([key, value]) => (
                ...
                <CardFooter className="flex gap-2 flex-wrap">
                  <Button
                    onClick={() =>
                      onConfirmation?.(true, false, req.action.data)
                    }
                  ...
                  <Button
                    onClick={() =>
                      onConfirmation?.(true, true, req.action.data)
                    }
                    variant="outline"
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    {req.plan.questions.map((q: any, qIndex: number) => (
                      <div
                        key={qIndex}
                        className="rounded-md bg-white dark:bg-purple-950/50 p-3 space-y-2"
                      >
                        <div className="flex items-start gap-2">
                          <Badge
                            variant="outline"
                            className="mt-0.5 border-purple-600 text-purple-700 dark:border-purple-400 dark:text-purple-300"
                          >
                            {qIndex + 1}
                          </Badge>
                      ...
                      <div className="rounded-md bg-white dark:bg-purple-950/50 p-3 space-y-2">
                        ...
                        <div className="space-y-1">
                          ...
                          <ol className="list-decimal list-inside space-y-1">
                            {req.plan.suggestedAction.steps.map(
                              (step: string, sIndex: number) => (
                                <li
                                  key={sIndex}
        ...
        {message.role === "assistant" && clarificationRequests.length > 0 && (
          <>
            {clarificationRequests.map((req, i) => (
              <Card
                key={`${message.id}-clarification-${i}`}
              ...
              >
                ...
                <CardContent className="space-y-3">
                  ...
                  {req.clarification.suggestedAnswers &&
                    req.clarification.suggestedAnswers.length > 0 && (
                      <div className="space-y-2">
                        <p className="text-xs text-blue-700 dark:text-blue-300">
                          Suggested answers:
                        </p>
                        <div className="flex flex-wrap gap-2">
                          {req.clarification.suggestedAnswers.map(
